...
@BadBoy_cmd(pattern="song ?(.*)")
async def _(event):
    BadBoy_bot = event.client
    try:
        await BadBoy_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in BadBoy_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await BadBoy_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
